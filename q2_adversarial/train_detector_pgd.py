"""
Q2(ii-a): Adversarial Detector using PGD attack via IBM ART.
Train ResNet-34 binary classifier to detect PGD adversarial images.

Usage:
    python train_detector_pgd.py --resnet18_path ./q2_adversarial/weights/resnet18_cifar10_best.pth
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train PGD adversarial detector")
    ...
    parser.add_argument("--pgd_eps_step", type=float, default=0.007)
    parser.add_argument("--pgd_max_iter", type=int, default=40)
...
def generate_pgd_adversarial(model, x_data, args, device):
    """Generate PGD adversarial examples using ART."""
    ...
    attack = ProjectedGradientDescent(
        ...
        eps=args.pgd_eps,
        eps_step=args.pgd_eps_step,
        max_iter=args.pgd_max_iter,
        batch_size=128,
    )

    print(f"[PGD] Generating adversarial examples "
          f"(eps={args.pgd_eps}, steps={args.pgd_max_iter})...")
    x_adv = attack.generate(x=x_data)
    print(f"[PGD] Generated {x_adv.shape[0]} adversarial samples")
...
def create_detector_dataset(x_clean, x_adv, val_split=0.1):
    """
    Create a mixed clean+adversarial dataset for binary classification.
    ...
    y_combined = np.concatenate([
        np.zeros(n, dtype=np.int64),  # clean
        np.ones(n, dtype=np.int64),   # adversarial
    ])
...
def train_detector(model, train_loader, val_loader, args, device, run_name):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save({
                ...
                'epoch': epoch,
                'val_acc': val_acc,
            }, os.path.join(args.weights_dir, f"detector_pgd_best.pth"))
...
def main():
    ...
    # Load the target model (ResNet-18 trained on clean CIFAR-10)
    ...
    # Generate PGD adversarial examples from training data
    x_adv_train = generate_pgd_adversarial(target_model, x_train, args, device)

    # Save adversarial examples
    os.makedirs(args.output_dir, exist_ok=True)
    np.save(os.path.join(args.output_dir, "pgd_adv_train.npy"), x_adv_train[:1000])
    ...
    # Create detector model (ResNet-34)
    detector = create_resnet34(num_classes=2).to(device)

    # WandB
    run = setup_wandb(
        project_name="Assignment-5",
        run_name="Q2_Detector_PGD",
        config={**vars(args), "attack": "PGD"}
    )

    # Train
    history, best_val_acc = train_detector(
        detector, train_loader, val_loader, args, device, "PGD_Detector"
    )

    # Plot
    plot_training_curves(
        history,
        os.path.join(args.output_dir, "detector_pgd_curves.png"),
        "PGD Detector (ResNet-34)"
    )

    if best_val_acc >= 70.0:
        print(f"\n[SUCCESS] PGD Detector: {best_val_acc:.2f}% >= 70% target!")
    else:
        print(f"\n[WARNING] PGD Detector: {best_val_acc:.2f}% < 70% target!")

    # Save history
    with open(os.path.join(args.output_dir, "detector_pgd_history.json"), 'w') as f:
